...
my_filter7 = KalmanFilter(dim_x=2, dim_z=1)
my_filter7.x = np.array([-60., -60.])       # initial state (location and velocity)
my_filter7.F = np.array([[1.,1.], [0.,1.]])    # state transition matrix
my_filter7.H = np.array([[1.,0.]])    # Measurement function
my_filter7.P *= 1023.                 # covariance matrix
my_filter7.R = 32                      # state uncertainty
my_filter7.Q = Q_discrete_white_noise(2, 1, .125) # process uncertainty
...
def rssi_kalman_filter_six():
	...
	for i in range(num):
		...
		my_filter6.predict()
		my_filter7.predict()
		R1, R2, R3, R4, R5, R6, R7 = kalman_six.RSSI_ave_six()
	...
	print("Average:%d, %d, %d, %d ,%d ,%d, %d" %(total1/num, total2/num, total3/num, total4/num, total5/num, total6/num, total7/num))
	return total1/num, total2/num, total3/num, total4/num, total5/num, total6/num, total7/num
